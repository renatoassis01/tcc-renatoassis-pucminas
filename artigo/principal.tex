...
\begin{abstract}
\noindent
A infraestrutura como código ou também conhecida como IaC, surgiu principalmente com a popularização de metodologias ágeis e computação na nuvem e além do movimento cultural denominado \textit{DevOps}. A \textit{IaC} define como fundamento a automatização de processos das operações de infraestrutura, utilizando uma abordagem para provisionar e gerenciar recursos de computação por meio de codificação ao invés do processo tradicional. Ao tratar a infraestrutura como código, as organizações podem usar as melhores práticas de desenvolvimento de software, incluindo revisão de código e controle de versão, documentação e testes da infraestrutura. Com \textit{Iac} novos conceito e ferramentas surgem, o que pode ser um dificultador para equipes inexperientes com computação na nuvem, pois trata-se uma nova abordagem e uma necessidade de mudança de paradigmas. Para adoção da infraestrutura como código, as equipes precisam escolher uma ferramenta ideal que atendam as necessidades. Neste artigo é proposto um estudo comparativo entre ferramentas de \textit{IaC}, e elenca aspectos positivos e negativos dessas ferramentas. 
